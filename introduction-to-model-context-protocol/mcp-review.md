...
Resources are controlled by your application code. Your app decides when to fetch resource data and how to use it — typically for UI elements or to add context to conversations. In the course project, resources were used in two ways:
...
You can see all three primitives in action in Claude's official interface: the workflow buttons demonstrate prompts, the Google Drive integration shows resources, and when Claude executes code or performs calculations, it's using tools behind the scenes.
