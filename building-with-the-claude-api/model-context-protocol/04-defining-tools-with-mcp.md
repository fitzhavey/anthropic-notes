...
In this example, we're creating an MCP server that manages documents stored in memory. The server provides two essential tools: one to read document contents and another to update them through find-and-replace operations.
...
The first tool allows Claude to read any document by its ID:
