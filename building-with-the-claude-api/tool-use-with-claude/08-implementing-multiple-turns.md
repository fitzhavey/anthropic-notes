...
Claude can request multiple tools in a single response. The message content contains a list of blocks, and we need to process each tool use block separately. The `run_tools` function filters for tool use blocks and processes each one:
...
This makes it easy to add new tools without modifying the core conversation logic.
